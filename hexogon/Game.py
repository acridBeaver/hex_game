...
class Game:
    ...
    def show_grid(self):
        ...
        for r in range(self.size):
            for c in range(self.size):
                x, y = self.coords(r, c)
                if self.state[r][c] == 1:
                    draw_hex(self.screen, GREEN, LIGHTYELLOW, (x, y), self.tile_size)
                elif self.state[r][c] == 2:
                    draw_hex(self.screen, BLUE, LIGHTYELLOW, (x, y), self.tile_size)
                elif self.state[r][c] == 3:
                    draw_hex(self.screen, LIGHTGREEN, LIGHTYELLOW, (x, y), self.tile_size)
                elif self.state[r][c] == 4:
                    draw_hex(self.screen, LIGHTBLUE, LIGHTYELLOW, (x, y), self.tile_size)
                else:
                    draw_hex(self.screen, DARKRED, LIGHTYELLOW, (x, y), self.tile_size)

    def checkWin(self):
        for y in range(self.size):
            if self.state[y][0] == 2:
                if check_move(Point(y, 0), self.state, lambda v: (v.Y == self.size - 1), 2):
                    return 2

        for x in range(self.size):
            if self.state[0][x] == 1:
                if check_move(Point(0, x), self.state, lambda v: (v.X == self.size - 1), 1):
                    return 1
        return 0
    ...
    def go_screen(self, winner):
        ...
        while go:
            ...
            self.show_grid()
            self.shadow()
            text_out(self.screen, 'GAME OVER', 80, ORANGE, (WIDTH / 2, HEIGHT / 3))
            if winner == 2:
                text_out(self.screen, 'Blue won', 60, BLUE, (WIDTH / 2, HEIGHT / 2))
            else:
                text_out(self.screen, 'Green won', 60, GREEN, (WIDTH / 2, HEIGHT / 2))
